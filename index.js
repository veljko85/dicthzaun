...
var createScene = function () {
  ...
  function changePosAndScaleFence(valueToCount, activeFence) {
    ...
    if (fencesArr[activeFence].type == "dich120180")
      fakeFences[activeFence].position.x = 0.3;

    if (
      fencesArr[activeFence].type == "dich90180" ||
      fencesArr[activeFence].type == "dichKosaDesna" ||
      fencesArr[activeFence].type == "dichKosaLeva"
    )
      fakeFences[activeFence].position.x = 0.45;

    if (fencesArr[activeFence].type == "dich150180")
      fakeFences[activeFence].position.x = 0.15;

    if (fencesArr[activeFence].type == "dich60180")
  ...
  //MAIN POST MESH //////////////////////////////////////////////////////////////////////////
  BABYLON.SceneLoader.ImportMeshAsync("", "mesh/", "mainPostDich.glb").then(
    (result) => {
      ...
      console.log(scene.getMaterialByID("Wood-Velja.001"));
  ...
  var createRightFence = (posX, posZ, rotY, type) =>
    BABYLON.SceneLoader.ImportMeshAsync("", "mesh/", "dichCombo12.glb").then(
      (result) => {
        ...
        fakeFence.position = new BABYLON.Vector3(
          getAbsPosX(result.meshes[1]) + 0.03,
          0.9,
        ...
        //POST CAP
        let rightPostCap = result.meshes[14];
        ...
        //POSTS
        let rightPost = result.meshes[15];
        ...
        //METAL PART TO HOLD WOOD POST
        let rightMetalPart = result.meshes[13];
        rightMetalPart.material = rootMat;
        metalParts.push(rightMetalPart);
        rightMetalParts.push(rightMetalPart);

        //SMALL METAL PART TO HOLD WOOD POST
        let rightSmallMetalPartFront = result.meshes[12];
        let rightSmallMetalPartBack = result.meshes[10];
        let rightSmallMetalPartBoth = result.meshes[11];
        ...
        //WOOD POST
        let rightWoodPost = result.meshes[17];
        ...
        //post roots
        let rightRoot = result.meshes[16];
        if (postType == 0) {
        ...
        foundationRightStart.position = new BABYLON.Vector3(
          getAbsPosX(result.meshes[16]),
          0.0001,
          getAbsPosZ(result.meshes[16])
        );
        ...
        //STRUMANKER
        let rightStrVord = result.meshes[18];
        ...
        ///sturmanker Ruck
        let rightStrRuck = result.meshes[19];
        ...
        rightPostCap.addChild(result.meshes[9]);
        rightPostCap.addChild(result.meshes[20]);
        ...
        let dich180180 = result.meshes[1];
        dich180180Arr.push(dich180180);
        dich180180.isVisible = false;

        let dich180150 = result.meshes[2];
        dich180150Arr.push(dich180150);
        dich180150.isVisible = false;

        let dich150180 = result.meshes[20];
        dich150180Arr.push(dich150180);
        dich150180.isVisible = false;

        let dich18090 = result.meshes[3];
        dich18090Arr.push(dich18090);
        dich18090.isVisible = false;

        let dich120180 = result.meshes[4];
        dich120180Arr.push(dich120180);
        dich120180.isVisible = false;

        let dich90180 = result.meshes[5];
        dich90180Arr.push(dich90180);
        dich90180.isVisible = false;

        let dich60180 = result.meshes[6];
        dich60180Arr.push(dich60180);
        dich60180.isVisible = false;

        let dichKosaDesna = result.meshes[8];
        dichKosaDesnaArr.push(dichKosaDesna);
        dichKosaDesna.isVisible = false;

        let dichKosaLeva = result.meshes[7];
        dichKosaLevaArr.push(dichKosaLeva);
        dichKosaLeva.isVisible = false;

        let dich180120 = result.meshes[9];
        dich180120Arr.push(dich180120);
        dich180120.isVisible = false;
        ...
        if (type == "dich180180") dich180180.isVisible = true;

        if (type == "dich180150") dich180150.isVisible = true;

        if (type == "dich150180") {
          dich150180.isVisible = true;
          setSmallerFenceSize(
            rightPost,
            rightPostCap,
            foundationRight,
        ...
        if (type == "dichKosaDesna") {
          dichKosaDesna.isVisible = true;
          rightWoodPost.scaling.y = 0.524;
          rightWoodPost.position.y = 0.532;
          rightPost.scaling.z = 0.524;
          rightPost.position.y = 0.504;
        ...
        if (type == "dichKosaLeva") {
          dichKosaLeva.isVisible = true;
          setSmallerFenceSize(
            rightPost,
            rightPostCap,
            foundationRight,
        ...
        if (type == "dich150180") fenceSizeObj = 150;

        if (type == "dich120180") fenceSizeObj = 120;

        if (type == "dich60180") fenceSizeObj = 60;
        ...
        //for main post
        if (fenceId > 0 && typeof activeFence == "boolean") {
          if (type != "dich18090" && type != "dichKosaLeva") {
  ...
  function handleTabActivnes() {
    if (!document.hidden) {
      createRightFence(0.99, 0, 0, "dich180180");
  ...
  let addNewFence180150 = document.getElementById("new-fence-180150");
  addNewFence180150.onclick = () => {
    createNewFence(
      createRightFence,
      activeArrowSide,
      rightPosts,
  ...
  let addNewFence18090 = document.getElementById("new-fence-18090");
  addNewFence18090.onclick = () => {
    createNewFence(
      createRightFence,
      activeArrowSide,
      rightPosts,
  ...
  let addNewFence120180 = document.getElementById("new-fence-120180");
  addNewFence120180.onclick = () => {
    createNewFence(
      createRightFence,
      activeArrowSide,
      rightPosts,
  ...
  let addNewFence180120 = document.getElementById("new-fence-180120");
  addNewFence180120.onclick = () => {
    createNewFence(
      createRightFence,
      activeArrowSide,
      rightPosts,
  ...
  function setPhostenType(a, b) {
    ...
    for (let i = 0; i < fencesArr.length; i++) {
      if (
        fencesArr[i].type == "dich18090" ||
        fencesArr[i].type == "dichKosaDesna"
      ) {
    ...
    for (let i = 0; i < fencesArr.length; i++) {
      if (
        ...
        for (let j = 0; j < fencesArr[i].children.length; j++) {
          if (
            fencesArr[fencesArr[i].children[j]].type != "dich18090" &&
            fencesArr[fencesArr[i].children[j]].type != "dichKosaLeva"
          ) {
  ...
  function changeFenceFunction(c, d, h) {
    ...
    if (d == "dich180180") dich180180Arr[h].isVisible = true;

    if (d == "dich180150") dich180150Arr[h].isVisible = true;

    if (d == "dich150180") dich150180Arr[h].isVisible = true;

    if (d == "dich18090") dich18090Arr[h].isVisible = true;

    if (d == "dich120180") dich120180Arr[h].isVisible = true;

    if (d == "dich90180") dich90180Arr[h].isVisible = true;

    if (d == "dich60180") dich60180Arr[h].isVisible = true;

    if (d == "dichKosaDesna") dichKosaDesnaArr[h].isVisible = true;

    if (d == "dichKosaLeva") dichKosaLevaArr[h].isVisible = true;

    if (d == "dich180120") dich180120Arr[h].isVisible = true;
    ...
    if (d == "dich18090") {
      ...
      for (let i = 0; i < fencesArr[h].children.length; i++) {
        if (
          fencesArr[fencesArr[h].children[i]].type != "dich18090" &&
          fencesArr[fencesArr[h].children[i]].type != "dichKosaLeva"
        ) {
          childType += 1;
        }
      }
      ...
      if (h > 0 && fencesArr[h].parent != undefined) {
        if (
          fencesArr[fencesArr[h].parent].type == "dich18090" ||
          fencesArr[fencesArr[h].parent].type == "dichKosaDesna"
        ) {
          let fenceSibling = 0;
          for (
            let i = 0;
  ...
  changeFence[0].addEventListener("click", () => {
    changeFenceFunction(180, "dich180180", activeFence);
  });
  changeFence[1].addEventListener("click", () => {
    changeFenceFunction(180, "dich180150", activeFence);
  });
  changeFence[2].addEventListener("click", () => {
    changeFenceFunction(150, "dich150180", activeFence);
  });
  changeFence[3].addEventListener("click", () => {
    changeFenceFunction(180, "dich18090", activeFence);
  });
  changeFence[4].addEventListener("click", () => {
    changeFenceFunction(120, "dich120180", activeFence);
  });
  changeFence[5].addEventListener("click", () => {
    changeFenceFunction(90, "dich90180", activeFence);
  });
  changeFence[6].addEventListener("click", () => {
    changeFenceFunction(60, "dich60180", activeFence);
  });
  changeFence[7].addEventListener("click", () => {
    changeFenceFunction(90, "dichKosaDesna", activeFence);
  });
  changeFence[8].addEventListener("click", () => {
    changeFenceFunction(90, "dichKosaLeva", activeFence);
  });
  changeFence[9].addEventListener("click", () => {
    changeFenceFunction(180, "dich180120", activeFence);
  });
    ...
    if (fencesArr[activeFence].type == "dich180180") {
      for (let i = 0; i < fencesArr.length; i++) {
        if (activeFence != i && fencesArr[i].status != "disposedFence") {
          changeFenceFunction(180, "dich180180", i);
        }
      }
    }
    if (fencesArr[activeFence].type == "dich180150") {
      for (let i = 0; i < fencesArr.length; i++) {
        if (activeFence != i && fencesArr[i].status != "disposedFence") {
          changeFenceFunction(180, "dich180150", i);
        }
      }
    }
    if (fencesArr[activeFence].type == "dich150180") {
      for (let i = 0; i < fencesArr.length; i++) {
        if (activeFence != i && fencesArr[i].status != "disposedFence") {
          changeFenceFunction(150, "dich150180", i);
        }
      }
    }
    if (fencesArr[activeFence].type == "dich18090") {
      for (let i = 0; i < fencesArr.length; i++) {
        if (activeFence != i && fencesArr[i].status != "disposedFence") {
          changeFenceFunction(180, "dich18090", i);
        }
      }
    }
    if (fencesArr[activeFence].type == "dich120180") {
      for (let i = 0; i < fencesArr.length; i++) {
        if (activeFence != i && fencesArr[i].status != "disposedFence") {
          changeFenceFunction(120, "dich120180", i);
        }
      }
    }
    if (fencesArr[activeFence].type == "dich90180") {
      for (let i = 0; i < fencesArr.length; i++) {
        if (activeFence != i && fencesArr[i].status != "disposedFence") {
          changeFenceFunction(90, "dich90180", i);
        }
      }
    }
    if (fencesArr[activeFence].type == "dich60180") {
      for (let i = 0; i < fencesArr.length; i++) {
        if (activeFence != i && fencesArr[i].status != "disposedFence") {
          changeFenceFunction(60, "dich60180", i);
        }
      }
    }
    if (fencesArr[activeFence].type == "dichKosaDesna") {
      for (let i = 0; i < fencesArr.length; i++) {
        if (activeFence != i && fencesArr[i].status != "disposedFence") {
          changeFenceFunction(90, "dichKosaDesna", i);
        }
      }
    }
    if (fencesArr[activeFence].type == "dichKosaLeva") {
      for (let i = 0; i < fencesArr.length; i++) {
        if (activeFence != i && fencesArr[i].status != "disposedFence") {
          changeFenceFunction(90, "dichKosaLeva", i);
        }
      }
    }
    if (fencesArr[activeFence].type == "dich180120") {
      for (let i = 0; i < fencesArr.length; i++) {
        if (activeFence != i && fencesArr[i].status != "disposedFence") {
          changeFenceFunction(180, "dich180120", i);
        }
      }
    }
  ...
  function deleteFence(a) {
    ...
    if (
      ...
      (fencesArr[fencesArr[a].parent].type == "dich18090" ||
        fencesArr[fencesArr[a].parent].type == "dichKosaDesna")
    ) {
      let childTypee = 0;
